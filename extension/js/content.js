...
function keywordsHighlighter(options, remove) {
  ...
  function highlight(node, pos, keyword, options, style) {
    ...
    span.className = 'highlighted' + ' ' + (options.subtleHighlighting ? 'subtle ' : '') + (options.invertedHighlighting ? 'inverted' : '') + 'style-' + style;
  ...
  if ('returnOptions' === request.message) {
    if ('undefined' != typeof request.keywords && request.keywords) {
      keywordsHighlighter({
		...
		'invertedHighlighting' : request.invertedHighlighting
      },
      request.remove
      );
    }
